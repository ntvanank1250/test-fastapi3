...
async def sign_in(request: Request):
    user_id = config.check_session(request=request, user_id=request.session.get("id"))
    if user_id:
...
async def login(request: Request, email: str = Form(...), password: str = Form(...), db: Session = Depends(utils.get_db)):
...
async def sign_up(request: Request, name: str = Form(...), email: str = Form(...), password: str = Form(...), repassword: str = Form(...), db: Session = Depends(utils.get_db)):
    ...
    new_user = config.create_user(name=name, email=email,password=password)
